...
def search_string_split(String):
    elements = []
...
def platform_name(platform_string):
    if platform_string == "":
...
def Search_API (platform_name, results_type):
    ...
    List_of_results_types = ['structures','references','info','Links']
    ...
    return optimade_platform_URL_structures


def searching(search_string, platform_name,results_type):

    URL_optimade = Search_API (platform_name, results_type)
    print(URL_optimade)
    searched_results=[]
    ...
    res = requests.get(URL_optimade, params=dict(filter=filters, page_limit=10)).json()
