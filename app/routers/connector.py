...
async def search_results(
    ...
    if platform_name =="Pubchem":
        res = requests.get(f"{search_url_base}{search_string}/record/JSON?callback=pubchem_callback",
        params=dict(page_limit=10)).json()
...
def search_string_split(search_string:str) -> List:
    elements = []
    for term in [a for a in re.split(",|\s+", search_string.strip()) if a]:
        try:
