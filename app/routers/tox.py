...
#python simple_api.py -t smiles -m acute_tox,tox,ALL_MODELS -o out.csv "CCC(=C(C1=CC=CC=C1)C2=CC=C(C=C2)OCCN(C)C)C3=CC=CC=C3"
...
def request_data(inputs):
    ...
    log(models)
    r=requests.post("http://tox.charite.de/protox_II/src/api_enqueue.php",data={'input_type':input_type,'input':inputs,'requested_data':json.dumps(models)},verify=False) #encode array, the rest are single strings
...
while task_id_list: #As long as we still have elements to get :
  ...
  for task_id in task_id_list:
    ...
    r=requests.post("http://tox.charite.de/protox_II/src/api_retrieve.php",data={'id':task_id},verify=False)
    if (r.status_code==200): #Data response
      if (r.text==""):
